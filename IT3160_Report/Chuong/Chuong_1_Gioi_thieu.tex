...
\begin{itemize}
    ...
    \item Giao diện \texttt{admin} để mô phỏng tình huống vận hành; scenario được lưu trong \texttt{localStorage} của từng trình duyệt và được gửi kèm khi người dùng tính route.
...
\]
là tập các tuyến. Nhiệm vụ của hệ thống là xây dựng một hành trình hợp lệ nối từ $p_s$ đến $p_t$ thông qua mạng MRT sao cho chi phí tổng quát là nhỏ nhất. Với các cặp điểm rất gần nhau, hệ thống cũng xét phương án \textit{walk-only}; phương án metro chỉ được chọn khi lợi ích thời gian đủ rõ so với việc đi bộ trực tiếp.
...
Trong lõi A* của route engine, hệ thống không tối ưu một chi phí vô hướng duy nhất mà tối ưu một bộ chi phí theo thứ tự từ điển. Với một hành trình $P$ giữa hai ga đã chọn, ta định nghĩa:
\[
\mathbf{C}(P) = \bigl(T(P), W(P), N_{tr}(P), N_{stop}(P)\bigr)
\]
trong đó:
\begin{itemize}
    \item $T(P)$ là tổng chi phí thời gian dùng để so sánh trong route engine;
...
\]
trong đó $\mathcal{P}(s_{\text{start}}, s_{\text{end}})$ là tập tất cả các hành trình hợp lệ nối giữa hai ga đã chọn và ký hiệu $\min^{\text{lex}}$ chỉ phép so sánh theo thứ tự ưu tiên từ điển. Cụ thể, hệ thống ưu tiên chi phí thời gian nhỏ nhất; nếu nhiều phương án có cùng chi phí thì ưu tiên phương án có ít thời gian đi bộ hơn; sau đó mới xét đến số lần chuyển tuyến và số chặng đi tàu. Riêng ở tầng API, các cặp ga đầu/cuối được đánh giá bằng hàm tiện ích có thêm độ khó chịu khi đi bộ và phạt chuyển tuyến, giúp tránh các route có thời gian tàu ngắn nhưng bắt người dùng đi bộ quá xa.
...
\]
Đại lượng này được dùng cho hiển thị và giải thích kết quả tới người dùng. Nó không nhân hệ số phạt đi bộ; các hệ số phạt chỉ được dùng trong bước lựa chọn route.
...
\begin{itemize}
    ...
    \item \textbf{transfer edge}: $(\texttt{transfer\_sec} + \texttt{line\_switch\_penalty},\; 0,\; 1,\; 0)$,
    \item \textbf{walk-transfer edge}: $(\lambda_{walk}\texttt{duration\_sec},\; \texttt{duration\_sec},\; 0,\; 0)$.
\end{itemize}
Trong dữ liệu địa lý hiện tại, thời gian chạy tàu giữa hai ga liên tiếp được nội suy theo tốc độ \(80\,km/h\), còn tốc độ đi bộ mặc định là \(1.1\,m/s\). Với cạnh transfer, \texttt{transfer\_sec} biểu diễn thời gian chuyển tuyến cơ sở, còn \texttt{line\_switch\_penalty} là phần phạt thêm trong hàm chi phí khi đổi tuyến. Walk-transfer nội bộ giữa các ga gần nhau vẫn bị phạt mạnh trong A* để hệ thống không lạm dụng đi bộ thay cho metro.

Như vậy, thuật toán không tối ưu một scalar cost duy nhất, mà tối ưu theo thứ tự ưu tiên từ điển trên bộ chi phí
\[
(T, W, N_{tr}, N_{stop}).
\]
Nói cách khác, hệ thống ưu tiên hành trình có chi phí thời gian nhỏ nhất; nếu nhiều phương án có cùng chi phí thì ưu tiên phương án có ít thời gian đi bộ hơn; nếu vẫn bằng nhau thì ưu tiên ít chuyển tuyến hơn; cuối cùng mới xét đến số chặng đi tàu.

Thuật toán nền được sử dụng là A* Search với hàng đợi ưu tiên. Dijkstra được xem là baseline lý thuyết vì A* sẽ trở thành Dijkstra nếu heuristic bằng 0. Trong dự án này, mỗi ga có toạ độ địa lý nên hệ thống dùng khoảng cách Haversine tới ga đích chia cho tốc độ tàu tối đa \(80\,km/h\) làm heuristic. Trên bản dữ liệu demo hiện hành, sau khi đồng bộ topology với lớp GIS, mạng định tuyến có khoảng hơn một trăm ga, nhiều trạng thái station-line, các segment MRT, transfer và một số walk-transfer. Với quy mô này, độ phức tạp xấp xỉ
\[
O((|V'| + |E'|)\log |V'|)
\]
...
\begin{tabular}{|l|l|>{\raggedright\arraybackslash}p{5cm}|>{\raggedright\arraybackslash}p{3.5cm}|}
\hline
\textbf{Loại cạnh} & \textbf{Ký hiệu} & \textbf{Cơ chế tính trọng số (\(w\))} & \textbf{Dữ liệu nguồn} \\ \hline
Ride & \(E_{ride}\) & Thời gian chạy tàu giữa hai node ga kế tiếp & \texttt{network\_topology.json}; \texttt{lines.geojson} dùng cho hình học hiển thị \\ \hline
Transfer & \(E_{transfer}\) & Thời gian chuyển tuyến trong cùng ga, cộng thêm phạt đổi tuyến khi so sánh route & \texttt{network\_topology.json} và auto-transfer từ các ga có nhiều tuyến \\ \hline
Walk-transfer & \(E_{walk\_trans}\) & Kết nối đi bộ liên ga hoặc bypass khi có scenario khoá đoạn & \texttt{network\_topology.json}, \texttt{subway\_osm\_enrichment.json} và auto proximity \\ \hline
\end{tabular}
\end{table}
