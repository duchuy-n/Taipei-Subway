...
Backend đóng vai trò lớp xử lý trung tâm, nơi tiếp nhận request, tính toán route và quản lý trạng thái runtime.
...
\begin{enumerate}
    ...
    \item Frontend gửi yêu cầu tới backend qua endpoint \texttt{/api/gis/route/points}.
    \item Backend nạp mạng tuyến, lớp ga GIS, lớp đường đi bộ và các cấu hình runtime cần thiết.
...
Bên cạnh luồng định tuyến chính, Hình~\ref{fig:seq_admin_scenario} mô tả cách Admin Console gửi scenario mới, backend phân tích phạm vi ảnh hưởng, cập nhật runtime constraint và làm mới cache để các truy vấn định tuyến sau đó phản ánh đúng trạng thái vận hành.
...
\begin{tikzpicture}[x=2.25cm,y=0.9cm,font=\small]
    ...
    \node[draw, rounded corners, fill=yellow!18, minimum width=2.4cm, minimum height=0.8cm] (cache) at (10.8,\TopY) {Runtime Cache/Data};
    ...
    \draw[->, thick] (2.6,-2.2) -- node[midway, above, fill=white, inner sep=1.2pt]{PUT /api/admin/scenarios} (5.2,-2.2);
    \draw[->, thick] (5.2,-3.2) -- node[midway, above, fill=white, inner sep=1.2pt]{Tính segment bị ảnh hưởng} (7.9,-3.2);
    \draw[->, thick] (7.9,-4.2) -- node[midway, above, fill=white, inner sep=1.2pt]{Đọc topology + GIS hiện hành} (10.8,-4.2);
    \draw[->, thick] (10.8,-5.2) -- node[midway, above, fill=white, inner sep=1.2pt]{Kết quả phân tích} (7.9,-5.2);
    \draw[->, thick] (7.9,-6.2) -- node[midway, above, align=center, fill=white, inner sep=1.2pt]{Lưu scenario\\+ invalidate cache} (5.2,-6.2);
    \draw[->, thick] (5.2,-7.2) -- node[midway, above, fill=white, inner sep=1.2pt]{Trả runtime state mới} (2.6,-7.2);
    \draw[->, thick] (2.6,-8.0) -- node[midway, above, fill=white, inner sep=1.2pt]{Scenario hiển thị trên bản đồ} (0,-8.0);

    \node[draw, fill=yellow!8, rounded corners, align=left, text width=3.6cm] at (10.8,-9.1) {\scriptsize Cache được làm mới để mọi truy vấn route tiếp theo dùng đúng trạng thái scenario mới nhất.};
\end{tikzpicture}%
}
\caption{Sequence Diagram cho luồng áp dụng admin scenario trong runtime}
...
Thay vào đó, route được chọn theo chi phí tổng quát:
\[
C = T_{ride} + T_{transfer} + \lambda_{walk}T_{walk} + \lambda_{switch}N_{switch}
\]
Trong cài đặt hiện tại, tốc độ đi bộ mặc định là \(1.3\,m/s\), tốc độ tàu dùng khi cần nội suy là \(80\,km/h\), và hệ số phạt đi bộ \(\lambda_{walk}\) là 5.
Cách đặt trọng số này giúp hệ thống vẫn hỗ trợ đi bộ để kết nối điểm người dùng với ga, nhưng không lạm dụng đi bộ thay cho metro.
...
tổ chức route engine và GIS services; xử lý admin scenario; viết test cho API và logic định tuyến.
\\
\hline
...
Nhóm này phụ trách toàn bộ phần xử lý nghiệp vụ phía máy chủ trên nền FastAPI, bao gồm tổ chức API, nạp dữ liệu runtime, xây dựng route engine và áp dụng các ràng buộc vận hành do khu vực admin tạo ra.
...
\begin{itemize}
    ...
    \item triển khai runtime cache để giảm chi phí nạp dữ liệu lặp lại và hỗ trợ làm mới trạng thái khi scenario thay đổi;
