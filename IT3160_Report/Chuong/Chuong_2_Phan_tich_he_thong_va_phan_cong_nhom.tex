...
\centering
    \begin{tikzpicture}[node distance=2.5cm, scale=0.85, transform shape]
        ...
        \node[comp] (comp_d3) at ([yshift=-0.4cm]data.center) [xshift=3.2cm] {OSM Raster};
...
Thay vào đó, lõi A* chọn route trong đồ thị MRT bằng phép so sánh từ điển trên vector chi phí:
\[
\mathbf{C}_{route} = (T,\; W,\; N_{tr},\; N_{stop})
\]
Trong cài đặt hiện tại, tốc độ đi bộ mặc định là \(1.1\,m/s\), tốc độ tàu dùng khi cần nội suy là \(80\,km/h\). Walk-transfer giữa hai ga gần nhau trong A* có hệ số phạt riêng để không lạm dụng đi bộ thay cho metro. Ở tầng API, sau khi A* trả về route giữa từng cặp ga ứng viên, hệ thống dùng một điểm tiện ích vô hướng để chọn cặp ga vào/ra phù hợp hơn với người dùng:
\[
S_{\text{candidate}} = T_{\text{actual}} + 1.5T_{\text{walk}} + 120N_{\text{transfer}} + T_{\text{rain}}
\]
trong đó \(T_{\text{actual}}\) là thời gian thật dùng để hiển thị, \(1.5T_{\text{walk}}\) phản ánh độ mệt khi đi bộ, \(120N_{\text{transfer}}\) phạt chuyển tuyến ở mức hai phút cho mỗi lần đổi tuyến, còn \(T_{\text{rain}}\) là phần thời gian tăng thêm do rain zone trong scenario admin. Điểm này chỉ dùng để chọn candidate; route con giữa hai ga vẫn được tìm bằng A* trên vector chi phí.
...
Khi đưa vào hàng đợi ưu tiên, heuristic được biểu diễn dưới dạng vector \((h(v),0,0,0)\) để cộng được với cost tuple hiện tại.
...
\Begin{
    ...
    $dist[v] \leftarrow (+\infty,+\infty,+\infty,+\infty), \forall v \in V'$\;
    $dist[s] \leftarrow (0,0,0,0)$\;
    $open.push((h(s), dist[s], s))$\;

    \While{$open$ không rỗng}{
        $(f_u, cost_u, u) \leftarrow open.pop\_min()$\;
        \If{$cost_u > dist[u]$}{
            \textbf{continue}\;
        }
        \If{$station(u) = t$}{
        ...
        \ForEach{cạnh $(u, v)$ kề $u$}{
            $new\_cost \leftarrow add\_cost(dist[u], cost(u, v))$\;
            \If{$new\_cost < dist[v]$}{
                $dist[v] \leftarrow new\_cost$\;
                $prev[v] \leftarrow (u, cost(u, v))$\;
                $f_v \leftarrow add\_cost(dist[v], h(v))$\;
                $open.push((f_v, dist[v], v))$\;
            }
        }
    }
...
\hline
Transfer & Chuyển đổi trạng thái ga giữa các tuyến & $(T_{trans} + P_{switch}, 0, 1, 0)$ \\
\hline
...
new_cost = add_cost(curr_cost, edge.cost)
if next_state not in distances or new_cost < distances[next_state]:
    ...
    h = heuristic(next_state, goal)  # h = (h_time, 0, 0, 0)
    estimated_total = add_cost(new_cost, h)
    priority_key = (estimated_total, new_cost, next_state)
    priority_queue.push(priority_key)
\end{minted}
\end{tcolorbox}
